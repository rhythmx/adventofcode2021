...
class CaveGraph
    ...
    def is_revisitable(node)
        return false if node == "start" or node == "end"
        lcbytes = node.each_byte.find_all{|c| c >= 'a'.ord and c <= 'z'.ord}
        return lcbytes.size == 0
    end

    def find_paths_to_end(current_path, visited_lc)
        paths = []
        current_node = current_path.last
        new_visited = visited_lc.clone
        if !is_revisitable(current_node)
            return nil if new_visited.include?(current_node)
            new_visited.merge([current_node])
        end
...
#puts graph.find_paths_to_end(["start"], Set.new).map{|p| p.join("-")}.join("\n")
npaths = graph.find_paths_to_end(["start"], Set.new).size
